...
<table cellpadding="0" cellspacing="0" style="border-collapse:collapse" border="1">
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>Amostra</td>
      <td>Microdado</td>
      <td>Lê os microdados de famílias.</td>
      <td></td>
      <td></td>
      <td></td>
      <td>X</td>
...
The first time the user runs a function, **{censobr}** will download the file and store it locally. This way, the data only needs to be downloaded once. More info in the [Data cache section](https://ipea.github.io/censobr/articles/censobr.html#data-cache) below.
...
In this example we'll be calculating the proportion of people with higher education in different racial groups in the state of Rio de Janeiro. First, we need to use the `read_population()` function to download the population data set. Here, we will be using the public data of the 2022 census.
...
In this example, we are going to map the proportion of households connected to a sewage network in Brazilian municipalities in the year 2010. First, we can easily download the households data set with the `read_households()` function.
...
head(esg)
```
In order to create a map with these values, we are going to use the [{geobr} package](https://ipea.github.io/geobr/) to download the geometries of Brazilian municipalities.
...
In this final example, we're going to visualize how the amount of money people used to spend on rent in 2010 varies spatially across the metropolitan area of São Paulo.
...
By default, **{censobr}** files are saved in the sytem 'User' directory. However, you can run the function `set_censobr_cache_dir()` to set custom cache directory. This directory is persistent across R sessions.

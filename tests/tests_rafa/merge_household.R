...
mort <- censobr::read_mortality(year = 2010, as_data_frame = T)
df_household <- censobr::read_households(year = 2010, as_data_frame = T)

nrow(mort)
nrow(df_household)

a <- left_join(mort, df_household)
head(a)



mortA <- censobr::read_mortality(year = 2010, as_data_frame = F)
df_householdA <- censobr::read_households(year = 2010, as_data_frame = F)

aA <- left_join(mortA, df_householdA)


a |>
  group_by(code_state) |>
  summarise( t = mean(V6530, na.rm = T))

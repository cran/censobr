  ...
  if(year == 2010 & lang == 'pt'){ # nocov start
    ...
    if ('V3061' %in% cols) {
      arrw <- arrw |> mutate(V3061 = case_when(
        ...
        # IBGE's country list carries 8000826 under five names (Escocia,
        # Inglaterra, Irlanda do Norte, Pais de Gales, Reino Unido); the
        # microdata cannot tell them apart, so only the aggregate label is used
